    ...
    <main>
      ...
      <section className="adventureLogsHero">
        <div>
          ...
          <p>写真、音、一言、場所。残したいものだけを、冒険のあとにそっと保存します。</p>
        </div>
      </section>

      <section className="adventureLogsSummary" aria-label="Adventure Logs summary">
        <div><span>記録された冒険</span><strong>{logs.length}</strong></div>
        <div><span>写真のある記録</span><strong>{logs.filter((log) => log.logTypes.includes("Photo")).length}</strong></div>
        <div><span>音のある記録</span><strong>{logs.filter((log) => log.logTypes.includes("Audio")).length}</strong></div>
      </section>
          ...
          {logs.map((log) => (
            <a className="adventureLogCard" href={log.url} key={log.id} target="_blank" rel="noreferrer">
              <div className="adventureLogCardTop">
                ...
                <div><p>{log.logTypes.join(" · ") || "Memory"}</p><h2>{log.name}</h2></div>
              ...
              <dl className="adventureLogMeta">
                <div><dt>日時</dt><dd>{formatDate(log.loggedAt)}</dd></div>
                {log.location ? <div><dt>場所</dt><dd>{log.location}</dd></div> : null}
                <div><dt>メディア</dt><dd>{log.media.length}件</dd></div>
              </dl>
            </a>
          ))}
        </section>
